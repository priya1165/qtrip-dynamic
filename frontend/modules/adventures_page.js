...
function addAdventureToDOM(adventures) {
 ...
 let container = document.getElementById("data");
  ...
  let adventureCardParent = document.createElement("div")
   adventureCardParent.className = "col-6 col-lg-3 col-md-4"

  adventureCardParent.innerHTML = `
  ...
  container.append(adventureCardParent);
})
...
function filterFunction(list, filters) {
  ...
  if(filters.duration!== ""){
    ...
    let values = filters.duration.split("-")
    let low = parseInt(values[0])
    let high = parseInt(values[1])
...
function getFiltersFromLocalStorage() {
  ...
  let localStorageFilters = JSON.parse(localStorage.getItem("filters"))
...
function generateFilterPillsAndUpdateDOM(filters) {
  ...
  let pillsContainer = document.getElementById("category-list")
  
  filters["category"].forEach((category) => {
    const pill = document.createElement("div")
    pill.className = "category-filter"

    pill.innerHTML = `<div>${category}</div>`
